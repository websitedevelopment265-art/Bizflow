...
export default function ArticleGrid({ articles, sidebar }: ArticleGridProps) {
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Articles Column */}
        <div className={`lg:col-span-${sidebar ? '2' : '3'}`}>
          <div className={`grid grid-cols-1 ${sidebar ? 'md:grid-cols-2' : 'md:grid-cols-2 lg:grid-cols-3'} gap-8`}>
            {articles.map((article, index) => (
              <motion.article
                ...
                className="group flex flex-col bg-white border border-slate-100 rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 h-full"
              >
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <Link to={`/article/${article.id}`}>
                    <h3 className="text-xl font-bold text-slate-900 mb-2 group-hover:text-brand-blue transition-colors line-clamp-2">
                      {article.title}
                    </h3>
                  </Link>
